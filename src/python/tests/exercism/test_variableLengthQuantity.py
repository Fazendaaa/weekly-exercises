...
from weekly_exercises.exercism.variableLengthQuantity import VariableLengthQuantity
...
def test_smallest_quintuple_byte() -> None:
    assert VariableLengthQuantity().encode([0x10000000]) == [
        0x81,
        0x80,
        0x80,
        0x80,
...
def test_arbitrary_quintuple_byte() -> None:
    assert VariableLengthQuantity().encode([0xFF000000]) == [
        0x8F,
        0xF8,
        0x80,
        0x80,
...
def test_maximum_32_bit_integer_input() -> None:
    assert VariableLengthQuantity().encode([0xFFFFFFFF]) == [
        0x8F,
        0xFF,
        0xFF,
        0xFF,
...
def test_maximum_32_bit_integer() -> None:
    assert VariableLengthQuantity().decode([0x8F, 0xFF, 0xFF, 0xFF, 0x7F]) == [
        0xFFFFFFFF
    ]
