...
#     "63915" is from the series "639". So the answer is 162.
#
#   Exception messages
#
...
# a ValueError when your largest_product() function receives invalid input. The
# tests will only pass if you both raise the exception and include a message
# with it. Feel free to reuse your code from the series exercise!
#
...
def largestProduct(
    ...
    if not series or span > len(series):
        raise ValueError("span must not exceed string length")
    if span < 0:
        raise ValueError("span must not be negative")
    ...
    return max(
        prod(int(digit) for digit in combination)
        for combination in [
            series[index : index + span] for index in range(len(series) - span + 1)
        ]
    )
